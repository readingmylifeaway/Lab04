...
# Train/test split
tt_inds <- caret::createDataPartition(glass$type, p = 0.8)
...
```{r}
variables <- glass %>%
  select(RI, Na, Mg, Si, K, Ca, Ba, Fe, Al, Na)
...
test_grid <- expand.grid(
  ...
  Al = seq(from = .29 , to = 3.5, length = 100)
)
head(test_grid)
...
ggplot() +
  geom_raster(data = background_knn,
    mapping = aes(x = Na, y = Al, fill = type), alpha = 0.3) +
...
1. At a minimum, this will be a vector of values long enough to store the validation set classification error rate for each validation fold
...
for(i in 1:length(k_vector)) {
  k <- k_vector[i]
  for(j in 1:10) {
    cross_val_glass <- train_glass %>% slice(crossval_val_fold_inds[[j]])
    cross_train_glass <- train_glass %>% slice(-crossval_val_fold_inds[[j]])
    
    knn_fit <- train(
      form = type ~ RI + Na + Mg + Al + Si + K + Ca + Ba + Fe,
      data = cross_train_glass, 
      method = "knn", 
      preProcess = "scale",
      trControl = trainControl(method = "none"),
      tuneGrid = data.frame(k = 10))
    
    y_hats <- predict(knn_fit, newdata = cross_val_glass, type = "raw")
    
head(y_hats)

MCE$MCE[j] <- mean(y_hats != cross_val_glass$type)
  }
  cross_val_results$MCE_diff_k[i] <- mean(MCE$MCE)
}
...
So, basically if we had predicted the most common type we woud have a .625 error however, with the KNN the error was only .45 

Collaboration:

Mya and Sarah were lab partners

Resources: 

Amelia helped out immensely with the code! 
